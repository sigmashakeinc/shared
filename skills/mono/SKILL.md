...
2. **Synthesize a Meta-Prompt from the raw goal, then adopt it as the run's spec.** This is the entry contract: the typical workflow is `/mono <initial goal prompt>` → you expand that prompt into a compact **meta-prompt** → you execute the pipeline against it. Derive the meta-prompt from the raw input **plus** repo knowledge already in context (this skill's invariants, `CLAUDE.md`, recalled memory, the apparent surface); **expansion = clarify the intent and surface the constraints already in force — never invent scope.** Emit it compactly (it is a spec, not an essay) with these fields, then **auto-adopt and proceed straight into Phase 0** — there is no separate approval gate; the single Phase 2 `AskUserQuestion`, seeded by the meta-prompt's Open-questions field, is the one human checkpoint:
   ...
   - **Invariants in play** — which standing guardrails this goal touches (pricing/value-surface · power-safety · coverage floor · legal-entity · Go-is-shipped-runtime · open-channel-discontinued · …) so they are front-of-mind, not rediscovered late.
...
- *(cross-OS coordination in scope)* Classify each cross-OS leg + owning OS/peer + capability requirement; record the leg table; run mesh preflight (`curl :7919/health` + `/runners`) and mark any offline peer `BLOCKED` before dispatch; load `ref/mesh-orchestration.md`.
...
- *(cross-OS coordination in scope)* Cross-OS slices dispatched to their owning peer via `POST :7919/dispatch` with `govProfile` mandatory (governed Claude Code session on peer hardware — power-safe by offload); local leg stays gate-wrapped per normal; `sha` pinned to current `HEAD` on every dispatch.
